...
async function main() {
  ...
  for (const pr of prs) {
    ...
    // issueコメントのみ
    const issueComments = await fetchIssueComments(OWNER, REPO, pr.number);
  ...
  fs.writeFileSync('pr_issue_comments.csv', csvRows.join('\n'), 'utf8');
  console.log('CSVファイル(pr_issue_comments.csv)に出力しました');
}
